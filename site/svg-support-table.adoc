...
// TODO: Add an image for batik's incorrect text shaping.

* `batik` doesn't support anti-aliasing during `clipPath`, but we are ignoring this.
* `batik` doesn't support text shaping (probably an incorrect term),
...
* All libraries are rendering text a bit different. We don't treat this as an error.
